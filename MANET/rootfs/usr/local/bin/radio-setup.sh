...
After=sys-subsystem-net-devices-%i.device manet-wlan-apply-link-names.service
...
[Unit]
After=wifi-rfkill-unblock.service manet-wlan-apply-link-names.service mesh-boot-lobby.service
...
# Install the swap-safe rename service. systemd .link Name= silently fails
# when two interfaces need to swap names (both targets occupied). This script
# handles the swap via temp names before any wireless service starts.
cat << 'EOF' > /etc/systemd/system/manet-wlan-apply-link-names.service
[Unit]
Description=Apply MANET wlan interface name mapping (swap-safe)
DefaultDependencies=no
After=systemd-udevd.service local-fs.target
Before=network-pre.target batman-enslave.service hostapd.service radio-setup-rerun.service
...
systemctl enable manet-wlan-apply-link-names.service
...
if [ "$needs_rerun" -eq 1 ]; then
    ...
    if MANET_WLAN_APPLY_RAW_ROLE_FILES=1 /usr/local/bin/manet-wlan-apply-link-names.sh; then
        ...
        mapfile -t halow_ifaces < <(cat /var/lib/halow_if 2>/dev/null)
        mapfile -t nonmesh_ifaces < <(cat /var/lib/no_mesh_if 2>/dev/null)
        AP_INTERFACE=$(head -1 /var/lib/ap_interface 2>/dev/null || true)
        needs_rerun=0
    else
        echo " > Live rename failed — will complete after reboot"
    fi
fi

if [ "$needs_rerun" -eq 1 ]; then
    echo " > Scheduling post-reboot re-run."

cat << 'EOF' > /etc/systemd/system/radio-setup-rerun.service
[Unit]
Description=Re-run radio-setup after interface rename
After=multi-user.target manet-wlan-apply-link-names.service
...
After=sys-subsystem-net-devices-${WLAN}.device manet-wlan-apply-link-names.service
...
WantedBy=sysinit.target
EOF
if [ -f /usr/local/bin/led-boot.sh ]; then
...
After=network-online.target manet-wlan-apply-link-names.service ${AFTER_DEVICES} ${WANTS_SERVICES}
...
# Disable cores 2 and 3 (only if CPU hotplug is available)
ExecStart=-/bin/bash -c '[ -f /sys/devices/system/cpu/cpu2/online ] && echo 0 > /sys/devices/system/cpu/cpu2/online'
ExecStart=-/bin/bash -c '[ -f /sys/devices/system/cpu/cpu3/online ] && echo 0 > /sys/devices/system/cpu/cpu3/online'

# Restore on stop (or when election scripts call systemctl stop cpu-powersave)
ExecStop=-/bin/bash -c '[ -f /sys/devices/system/cpu/cpu2/online ] && echo 1 > /sys/devices/system/cpu/cpu2/online'
ExecStop=-/bin/bash -c '[ -f /sys/devices/system/cpu/cpu3/online ] && echo 1 > /sys/devices/system/cpu/cpu3/online'
...
if [ -f /var/lib/radio-setup-reboot-pending ]; then
    ...
    echo ""
    echo "=================================================="
    echo " Interface renames pending — skipping auto-reboot"
    echo " Run: sudo systemctl start radio-setup-rerun.service"
    echo " or reboot manually when ready"
    echo "=================================================="
fi
