...
REMOTE_VERSION_URL="https://raw.githubusercontent.com/very-srs/MANET/refs/heads/main/MANET/version.txt"
case "$BOARD" in
    rpi5)
...
if [ -z "$REMOTE_VERSION" ] && [ "$REMOTE_VERSION_URL" != "https://raw.githubusercontent.com/very-srs/MANET/refs/heads/main/MANET/version.txt" ]; then
    REMOTE_VERSION=$(curl -H 'Cache-Control: no-cache, no-store' \
        -H 'Pragma: no-cache' \
        -fs https://raw.githubusercontent.com/very-srs/MANET/refs/heads/main/MANET/version.txt | head -n 1 2>/dev/null)
fi
