// D3.js force-directed mesh topology — card-style nodes with link quality labels
var TOPO_CW = 154;
var TOPO_CH = 96;
...
function topoCardInner(d) {
  var name = escHtml(d.hostname || d.id);
  var badge = '';
  if (d.is_me) badge = '<span class="tc-pill tc-pill-me">ME</span>';
  else if (d.is_gateway) badge = '<span class="tc-pill tc-pill-gw">GW</span>';

  var rows = '';
  if (d.is_me) {
    if (d.ip) rows += tcRow('IP', escHtml(d.ip));
  } else if (d.stale) {
    rows += tcRow('Status', '<span style="color:#ef4444">OFFLINE</span>');
    if (d.last_seen) rows += tcRow('Last seen', fmtAge(d.last_seen, d._ts));
  } else {
    var pct = d.tq != null ? tqPct(d.tq) + '%' : '--';
    rows += tcRow('Quality', '<span style="color:' + tqColor(d.tq) + '">' + pct + '</span>');
    if (d.hop_count != null && d.hop_count > 0)
      rows += tcRow('Hops', d.hop_count);
  }
  if (d.battery && d.battery.percentage != null) {
    var bc = d.battery.percentage < 20 ? '#ef4444' : '#22c55e';
    rows += tcRow('Battery', '<span style="color:' + bc + '">' + d.battery.percentage + '%</span>');
  }

  return '<div class="tc-head">' + name + badge + '</div>' +
    '<div class="tc-rows">' + rows + '</div>';
}

function topoCardCls(d) {
  var cls = 'topo-card';
  if (d.is_me) cls += ' tc-self';
  if (d.is_gateway && !d.is_me) cls += ' tc-gw';
  if (d.stale) cls += ' tc-stale';
  return cls;
}

function tcRow(label, val) {
  return '<div class="tc-row"><span class="tc-lbl">' + label +
...
function topoUpdate(data) {
  ...
  var nodes = data.nodes.map(function(n, idx) {
    ...
    var obj = Object.assign({}, n, { stale: stale, _ts: ts });
    if (old) {
  ...
  var linkAll = linkEnter.merge(linkSel)
    ...
    .attr('stroke-opacity', function(d) {
      var s = d.source.id || d.source, t = d.target.id || d.target;
      if (staleIds.has(s) || staleIds.has(t)) return 0.3;
      return 0.7;
    });
  ...
  // --- Nodes ---
  var nodeSel = g.select('.nodes').selectAll('g.topo-node')
    .data(nodes, function(d) { return d.id; });
  nodeSel.exit().remove();

  var nodeEnter = nodeSel.enter().append('g').attr('class', 'topo-node');

  nodeEnter.append('rect')
    .attr('class', 'topo-hitbox')
    .attr('width', TOPO_CW).attr('height', TOPO_CH)
    .attr('x', -TOPO_CW / 2).attr('y', -TOPO_CH / 2)
  ...
  nodeAll.each(function(d) {
    var fo = d3.select(this).select('foreignObject').node();
    while (fo.firstChild) fo.removeChild(fo.firstChild);
    var div = document.createElementNS('http://www.w3.org/1999/xhtml', 'div');
    div.className = topoCardCls(d);
    div.innerHTML = topoCardInner(d);
    fo.appendChild(div);
  });

  nodeAll.style('cursor', function(d) { return d.ip ? 'pointer' : 'grab'; });

  // Tooltip (shows extra detail on hover)
  ...
  nodeAll
    .on('mouseover', function(event, d) {
      ...
      if (!d.is_me && d.tq != null)
        html += '<div class="tt-row"><span class="tt-label">TQ</span>' + d.tq + ' (' + tqPct(d.tq) + '%)</div>';
      if (d.hop_count)
        html += '<div class="tt-row"><span class="tt-label">Hops</span>' + d.hop_count + '</div>';
      if (d.uptime)
        html += '<div class="tt-row"><span class="tt-label">Up</span>' + d.uptime + '</div>';
      if (d.is_gateway)
        html += '<div class="tt-row"><span class="tt-label">Role</span>Gateway' +
          (d.is_selected_gw ? ' (selected)' : '') + '</div>';
      if (d.limp)
        html += '<div class="tt-row"><span class="tt-label">State</span><span style="color:var(--bad)">LIMP MODE</span></div>';
      if (d.ip)
        html += '<div class="tt-row" style="opacity:0.5;font-size:10px;margin-top:4px">Click for options</div>';
